...
let createAndGetDefault () =
    ...
    let copyBinariesOnly = BuildTask.create "CopyBinariesOnly" [] {
        !! "**/src/**/*.??proj"
        -- "**/src/**/*.shproj"
                    ...
                    let name, dInfo = 
                            d.Name, 
                                (DirectoryInfo.getSubDirectories d 
                                 |> Array.filter(fun x -> 
                                    x.FullName.ToLower().Contains("netcoreapp2.2")
                                    || x.FullName.ToLower().Contains("netstandard2.0")
                                  //  || x.FullName.ToLower().Contains("netstandard2.1")
                                 )
                                ).[0]
                    dInfo.GetFiles()
                    |> Array.filter (fun x -> 
    ...
    BuildTask.createEmpty "All" [build; generateDocs]
